...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # Pad input if given
        if pad:
            x = Fn.pad(x, [pad, pad, pad, pad], "constant", 0.)
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        inp = x[n, :, i * stride: i * stride + HH, j * stride:j * stride + WW]
                        out[n, f, i, j] = (inp * w[f]).sum() + b[f]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                out[:, :, i, j] = block.max(dim=-1)[0].max(dim=-1)[0]
    ...
    def backward(dout, cache):
        ...
        for i in range(H_out):
            for j in range(W_out):
                block = x[:, :, i * stride:i * stride + HH, j * stride:j * stride + WW]
                max_value = block.max(dim=-1, keepdim=True)[0].max(dim=-2, keepdim=True)[0]
                mask = (block == max_value)
                dx[:, :, i * stride:i * stride + HH, j * stride:j * stride + WW][mask] = dout[:, :, i, j].view(-1)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # One Conv Layer
        self.params["W1"] = torch.normal(0., weight_scale, dtype=dtype, device=device,
                                         size=[num_filters, input_dims[0], filter_size, filter_size])
        self.params["b1"] = torch.zeros([num_filters], dtype=dtype, device=device)

        # Two Linear Layers
        in_dims = num_filters * input_dims[1] // 2 * input_dims[2] // 2
        self.params["W2"] = torch.normal(0., weight_scale, [in_dims, hidden_dim], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        x, fc1_cache = Linear_ReLU.forward(x, W2, b2)
        scores, fc2_cache = Linear.forward(x, W3, b3)
        ...
        loss += self.reg * (self.params["W1"] ** 2).sum() + \
                self.reg * (self.params["W2"] ** 2).sum() + \
                self.reg * (self.params["W3"] ** 2).sum()

        dx, grads["W3"], grads["b3"] = Linear.backward(ds, fc2_cache)
        grads["W3"] += 2 * self.reg * self.params["W3"]
        dx, grads["W2"], grads["b2"] = Linear_ReLU.backward(dx, fc1_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self, input_dims=(3, 32, 32),
        ...
        # Initialize weights and biases for conv blocks
        kernel_size = 3
        macro_size = 2 if batchnorm else 1
        dims = [input_dims[0], *num_filters, num_classes]
        for i in range(self.num_layers):
            in_dims, out_dims = dims[i:i + 2]
            if i == self.num_layers - 1:
                down_stride = 2 ** len(set(self.max_pools))
                grid_height = input_dims[1] // down_stride
                grid_width = input_dims[2] // down_stride
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        macro_size = 2 if self.batchnorm else 1
        caches = []
        x = X
        for i in range(self.num_layers):
            args = []
            if i == self.num_layers - 1:
                ...
                layer_name = "Conv"
                if self.batchnorm:
                    gamma = self.params[f"BN{i * macro_size + 2}.gamma"]
                    beta = self.params[f"BN{i * macro_size + 2}.beta"]
                    bn_param = self.bn_params[i]
                    args.append(gamma)
                    args.append(beta)
                    args.append(conv_param)
                    args.append(bn_param)
                    if i in self.max_pools:
                        args.append(pool_param)
        ...
        for i in range(self.num_layers):
            if i == self.num_layers - 1:
                w = self.params[f"Linear{i * macro_size + 1}.w"]
            else:
                w = self.params[f"Conv{i * macro_size + 1}.w"]
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                dx, dw, db = Linear.backward(dx, cache)
                dw += 2 * self.reg * self.params[f"Linear{(i - 1) * macro_size + 1}.w"]
                grads[f"Linear{(i - 1) * macro_size + 1}.w"] = dw
                grads[f"Linear{(i - 1) * macro_size + 1}.b"] = db
            else:
                if self.batchnorm:
                    ...
                    if (i - 1) in self.max_pools:
                        ...
                        layer = Conv_ReLU
                    dx, dw, db = layer.backward(dx, cache)
                dw += 2 * self.reg * self.params[f"Conv{(i - 1) * macro_size + 1}.w"]
                grads[f"Conv{(i - 1) * macro_size + 1}.w"] = dw
                grads[f"Conv{(i - 1) * macro_size + 1}.b"] = db
