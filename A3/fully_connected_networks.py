...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        h, fc1_cache = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
        scores, fc2_cache = Linear.forward(h, self.params["W2"], self.params["b2"])
        ...
        ce_loss, ds = softmax_loss(scores, y)
        reg_loss = self.reg * torch.sum(self.params["W1"] ** 2) + \
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, device=device, num_epochs=50)
