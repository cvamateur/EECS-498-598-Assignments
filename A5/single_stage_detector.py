...
def GenerateAnchor(anc, grid):
    ...
    ##############################################################################
    B, h_amap, w_amap = grid.shape[:3]
    A = anc.shape[0]
    grid = grid.unsqueeze(1)  # [B, 1, H', W', 2]
    anc = anc.view(A, 1, 1, 2)  # [   A,  1,  1, 2]

    anchors = anc.new_zeros([B, A, h_amap, w_amap, 4])
    anchors[..., 0:2] = grid - anc * 0.5
    anchors[..., 2:4] = grid + anc * 0.5
...
def GenerateProposal(anchors, offsets, method='YOLO'):
    ...
    if method == "YOLO":
        proposals[..., 0:2] = 0.5 * (1. + offsets[..., 2:4].exp()) * anchors[..., 0:2] + \
                              0.5 * (1. - offsets[..., 2:4].exp()) * anchors[..., 2:4] + offsets[..., 0:2]
        proposals[..., 2:4] = 0.5 * (1. - offsets[..., 2:4].exp()) * anchors[..., 0:2] + \
                              0.5 * (1. + offsets[..., 2:4].exp()) * anchors[..., 2:4] + offsets[..., 0:2]
    else:
        proposals[..., 0:2] = 0.5 * (1. - 2. * offsets[..., 0:2] + offsets[..., 2:4].exp()) * anchors[..., 0:2] + \
                              0.5 * (1. + 2. * offsets[..., 0:2] - offsets[..., 2:4].exp()) * anchors[..., 2:4]
        proposals[..., 2:4] = 0.5 * (1. - 2. * offsets[..., 0:2] - offsets[..., 2:4].exp()) * anchors[..., 0:2] + \
                              0.5 * (1. + 2. * offsets[..., 0:2] + offsets[..., 2:4].exp()) * anchors[..., 2:4]
...
def IoU(proposals, bboxes):
    ...
    _, N = bboxes.shape[:2]
    proposals = proposals.view(B, -1, 4).unsqueeze(2)   # [B, AH'W', 1, 4]
    bboxes = bboxes.unsqueeze(1)                        # [B,     1, N, 5]

    # Area of proposals and bboxes
    proposals_wh = proposals[..., 2:4] - proposals[..., 0:2]
    area_proposals = proposals_wh[..., 0] * proposals_wh[..., 1]    # [B, AH'W', 1]
    bboxes_wh = bboxes[..., 2:4] - bboxes[..., 0:2]
    area_bboxes = bboxes_wh[..., 0] * bboxes_wh[..., 1]             # [B,     1, N]

    # Area of intersections
    intersect_xy_tl = torch.maximum(proposals[..., 0:2], bboxes[..., 0:2])
    intersect_xy_br = torch.minimum(proposals[..., 2:4], bboxes[..., 2:4])
    intersect_wh = (intersect_xy_br - intersect_xy_tl).clamp(min=0.)
    area_intersect = intersect_wh[..., 0] * intersect_wh[..., 1]    # [B, AH'W', N]

    # IoU
    union = area_proposals + area_bboxes - area_intersect
    iou_mat = area_intersect / union
